...
func (app *PeopleController) HandleWelcome(c *fiber.Ctx) error {
	...
	templates := utilities.NewTemplateStructures("top", "authentication/welcome.tmpl")
	templateData := utilities.NewTemplateData();

	// The email of the person being welcomed
	id, err :=c.ParamsInt("id")
	if err != nil {
		errMsg := "invalid or missing person identity"
		log.Println(errMsg)
		c.Status(http.StatusUnauthorized)
		templateData.AddData("Error", errMsg)
		templateData.AddData("Message","Try again")
		return utilities.RenderPage(c, templates.GetAllTemplateStructures(), templateData.GetAllData())
	}

	// Get the person name
	var person people.Person
	person, err = app.service.GetByID(uint(id))
	if err != nil {
		log.Println("Error fetching person by email:", err)
		c.Status(http.StatusUnauthorized)
		templateData.AddData("Error", "User not found")
		templateData.AddData("Message","Try again")
		return utilities.RenderPage(c, templates.GetAllTemplateStructures(), templateData.GetAllData())
